node {
	...
	def backend = sh(returnStatus: true, script: "helm ls | grep -q prometheus")
        ...
        PPOD_NAME = sh(script: "kubectl get pods --namespace monitoring -l \"app=prometheus-node-exporter,release=prometheus\" -o jsonpath=\"{.items[0].metadata.name}\"",returnStdout: true).trim()
	...
	SERVICE_IP = sh(script: "kubectl get svc $POD_NAME --namespace monitoring --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"
